class BridgeModule{constructor(t){this.app=t,this.el=null;try{this.collection=JSON.parse(localStorage.getItem("bridge_collection")||"[]")}catch(e){this.collection=[]}try{this.bridgeCount=parseInt(localStorage.getItem("bridge_count")||"0")}catch(e){this.bridgeCount=0}try{this.streakData=JSON.parse(localStorage.getItem("bridge_streak")||'{"count":0,"lastDate":""}')}catch(e){this.streakData={count:0,lastDate:""}}this.currentData=null,this.activeCategory=null,this.BRIDGE_META={direct:{label:"Do\u011Frudan",color:"#94a3b8",desc:"Birebir e\u015Fle\u015Fme"},transform:{label:"D\xF6n\xFC\u015F\xFCm",color:"#a78bfa",desc:"K\xFClt\xFCrel de\u011Fi\u015Fim"},multiply:{label:"\xC7o\u011Falma",color:"#34d399",desc:"\xC7oklu anlam"},disappear:{label:"Kaybolu\u015F",color:"#f87171",desc:"Kay\u0131p anlam"},emerge:{label:"T\xFCreme",color:"#22d3ee",desc:"Yeni anlam"}},this.EXAMPLES=["Can\u0131m s\u0131k\u0131ld\u0131","Kafam \xE7ok kar\u0131\u015F\u0131k","\u0130\u015Fler \xE7ok yo\u011Fun gidiyor","\xDCst\xFCmden b\xFCy\xFCk bir y\xFCk kalkt\u0131"]}init(t){this.el=t,this._render(),this._initOrientation()}_initOrientation(){try{screen.orientation&&screen.orientation.unlock&&screen.orientation.unlock()}catch(e){}const t=()=>{if(!document.fullscreenElement&&!document.webkitFullscreenElement){const e=document.documentElement,i=e.requestFullscreen||e.webkitRequestFullscreen;i&&i.call(e).catch(()=>{})}};t(),this._orientHandler=()=>t(),window.addEventListener("resize",this._orientHandler,{passive:!0}),window.attachQuickMenuTrigger&&window.attachQuickMenuTrigger(this.el)}destroy(){this._orientHandler&&(window.removeEventListener("resize",this._orientHandler),this._orientHandler=null);try{screen.orientation&&screen.orientation.lock&&screen.orientation.lock("portrait").catch(()=>{})}catch(t){}this.el&&(this.el.innerHTML="")}_render(){const t=Object.entries(this.BRIDGE_META).map(([r,n])=>`
      <div class="bridge-legend-item" title="${n.desc}">
        <div class="bridge-legend-dot" style="background:${n.color}"></div>
        <span>${n.label}</span>
      </div>
    ...
    `,this._bindEvents(),this._renderCollection()}_bindEvents(){const t=this.el.querySelector("#b-trigger"),e=this.el.querySelector("#b-text");t.addEventListener("click",()=>{this._smartDiscovery()}),e.addEventListener("keydown",i=>{if((i.ctrlKey||i.metaKey)&&i.key==="Enter"){const r=e.value.trim();r&&this._analyze(r)}}),this.el.querySelectorAll(".bridge-example-pill").forEach(i=>{i.addEventListener("click",()=>{e.value=i.dataset.ex,this._analyze(i.dataset.ex)})}),this.el.querySelectorAll(".bridge-cat-chip").forEach(i=>{i.addEventListener("click",()=>{this.activeCategory=i.dataset.cat||null,this.el.querySelectorAll(".bridge-cat-chip").forEach(r=>r.classList.remove("active")),i.classList.add("active")})}),this.el.querySelector("#b-quiz-btn").addEventListener("click",()=>this._showQuiz()),this.el.querySelector("#b-save-btn").addEventListener("click",()=>this._saveToColl()),this.el.querySelector("#b-shadow-btn").addEventListener("click",()=>this._startShadowing())}_smartDiscovery(){var l,o;if(!window.BRIDGE_DATA)return;const t=((o=(l=window._app)==null?void 0:l.state)==null?void 0:o.get("cefrLevel"))||"B1",i={A1:1,A2:5,B1:12,B2:18,C1:25,C2:30}[t]||12;let r=window.BRIDGE_DATA;if(this.activeCategory&&(r=r.filter(s=>s.category===this.activeCategory)),i<10){const s=r.filter(a=>a.register==="neutral"||a.bridges.some(d=>d.bridge_type==="direct"));s.length>=5&&(r=s)}else if(i<22){const s=r.filter(a=>a.register==="informal"||a.bridges.some(d=>d.bridge_type==="transform"));s.length>=5&&(r=s)}r.length||(r=window.BRIDGE_DATA);const n=r[Math.floor(Math.random()*r.length)];this.el.querySelector("#b-text").value=n.tr,window.analyticsManager&&window.analyticsManager._log("discovery_bridge",{tr:n.tr,level:i,category:n.category}),this._analyze(n.tr)}_analyze(t){if(!t||typeof findBridgeMatch!="function")return;const e=findBridgeMatch(t);if(e){this.currentData={originalTR:t,...e.entry},this._renderResult(),this.bridgeCount++;try{localStorage.setItem("bridge_count",this.bridgeCount)}catch(i){}this.el.querySelector("#b-stat-count").textContent=`\u2728 ${this.bridgeCount}`,this._updateStreak()}else UI.toast("Bu ifade hen\xFCz veritaban\u0131nda yok.")}_renderResult(){const t=this.currentData,e=this.el.querySelector("#b-result"),i=this.el.querySelector("#b-placeholder"),r=this.el.querySelector("#b-actions-area");i.style.display="none",e.style.display="block",r.style.display="flex",e.innerHTML=`
    ...
    `,e.querySelector("#b-speak-main").addEventListener("click",()=>this._speak(t.english_primary)),this._renderCards(t.bridges||[]),this._renderInsight(t.cultural_insight,t.fluency_tip)}_renderCards(t){const e=this.el.querySelector("#b-cards-area");e.innerHTML=t.map(i=>{var r,n;return`
      <div class="bridge-card-row">
        ...
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center;">
          ...
          <div style="font-size:0.6rem; margin-top:4px;">${((n=this.BRIDGE_META[i.bridge_type])==null?void 0:n.label)||""}</div>
        </div>
    ...
    `).join(""),t.querySelectorAll(".bridge-cat-chip").forEach(e=>{e.addEventListener("click",()=>{this.el.querySelector("#b-text").value=e.dataset.tr,this._analyze(e.dataset.tr)})})}_saveToColl(){if(!(!this.currentData||this.collection.some(t=>t.originalTR===this.currentData.originalTR))){this.collection.unshift(this.currentData);try{localStorage.setItem("bridge_collection",JSON.stringify(this.collection))}catch(t){}UI.toast("Koleksiyona eklendi!"),this._renderCollection()}}_speak(t){if(!window.speechSynthesis)return;window.speechSynthesis.cancel();const e=new SpeechSynthesisUtterance(t);e.lang="en-US",e.rate=.85,window.speechSynthesis.speak(e)}_updateStreak(){const t=new Date().toDateString(),e=new Date(Date.now()-864e5).toDateString();if(this.streakData.lastDate!==t){this.streakData.count=this.streakData.lastDate===e?this.streakData.count+1:1,this.streakData.lastDate=t;try{localStorage.setItem("bridge_streak",JSON.stringify(this.streakData))}catch(i){}this.el.querySelector("#b-stat-streak").textContent=`\u{1F525} ${this.streakData.count}`}}_startShadowing(){if(!this.currentData)return;const t=this.currentData.english_primary,e=document.createElement("div");e.style="position:fixed; inset:0; background:rgba(0,0,0,0.9); z-index:10000; display:flex; align-items:center; justify-content:center; padding:20px;",e.innerHTML=`<div class="bridge-panel" style="max-width:400px; text-align:center; padding:40px;">
      <h3>G\xF6lgeleme</h3>
    ...
    </div>`,document.body.appendChild(e),e.querySelector("#sh-close").onclick=()=>{window.speechSynthesis.cancel(),e.remove()};let i=0;const r=()=>{i>=3||!e.isConnected||(i++,e.querySelector("#sh-stat").textContent=`Tekrar ${i}/3 - Dinle ve S\xF6yle`,this._speak(t),setTimeout(r,4e3))};r()}_showQuiz(){if(!window.BRIDGE_DATA)return;const t=[...window.BRIDGE_DATA].sort(()=>.5-Math.random()).slice(0,5);let e=0,i=0;const r=document.createElement("div");r.style="position:fixed; inset:0; background:rgba(0,0,0,0.95); z-index:10000; display:flex; align-items:center; justify-content:center; padding:20px;",document.body.appendChild(r);const n=()=>{var s;if(e>=t.length){r.innerHTML=`<div class="bridge-panel" style="text-align:center; padding:40px;"><h2>Bitti!</h2><p style="font-size:2rem;">${i}/${t.length}</p><button class="bridge-cat-chip active" id="bq-close-btn">Kapat</button></div>`,r.querySelector("#bq-close-btn").addEventListener("click",()=>r.remove()),window.analyticsManager&&window.analyticsManager.lessonComplete("bridge",Math.round(i/t.length*100)),(s=this.app)!=null&&s.addXP&&this.app.addXP(i*20,"medium","bridge");return}const l=t[e],o=[l.english_primary,...window.BRIDGE_DATA.filter(a=>a.id!==l.id).sort(()=>.5-Math.random()).slice(0,2).map(a=>a.english_primary)].sort();r.innerHTML=`<div class="bridge-panel" style="max-width:500px; width:100%;">
        <div style="margin-bottom:20px;">Soru ${e+1}/5</div>
        <h3 style="margin-bottom:20px;">"${l.tr}" kar\u015F\u0131l\u0131\u011F\u0131 nedir?</h3>
        ${o.map(a=>`<button class="bridge-cat-chip active" style="display:block; width:100%; margin-bottom:10px; background:var(--bg-elevated);" data-o="${a}">${a}</button>`).join("")}
      </div>`,r.querySelectorAll("button[data-o]").forEach(a=>{a.onclick=()=>{a.dataset.o===l.english_primary?(i++,a.style.background="var(--green)"):a.style.background="var(--rose)",setTimeout(()=>{e++,n()},800)}})};n()}}window.BridgeModule=BridgeModule;
