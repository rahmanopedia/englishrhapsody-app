...
/* ── 18. Mod Bağlantısı: Kelime → Cümle Pratiği ── */
(function(){
  var _st = { words:[], idx:0, score:0, placed:[], remaining:[], checked:false };

  function _shuf(a){ return a.slice().sort(function(){ return Math.random()-0.5; }); }

  function start(wordPool){
    var pool = (wordPool||[]).filter(function(w){ return w && w.ex && w.ex.trim().split(/\s+/).length >= 3; });
    if(!pool.length) return;
    _st.words = _shuf(pool).slice(0,8);
    _st.idx = 0; _st.score = 0;
    _render();
  }

  function close(){
    var el = document.getElementById('exp-overlay');
    if(!el) return;
    el.style.opacity = '0';
  ...
  function _render(){
    var old = document.getElementById('exp-overlay');
    if(old) old.parentNode && old.parentNode.removeChild(old);
    if(_st.idx >= _st.words.length){ _showResult(); return; }

    var w = _st.words[_st.idx];
    var raw = (w.ex||'').replace(/[.!?]+$/, '');
    var parts = raw.split(/\s+/);
    _st.placed = [];
    _st.remaining = _shuf(parts.map(function(p,i){ return {w:p,i:i}; }));
    _st.checked = false;

    var pct = Math.round(_st.idx / _st.words.length * 100);
    var div = document.createElement('div');
    div.id = 'exp-overlay';
    div.className = 'exp-overlay';
    div.innerHTML =
      '<div class="exp-header">' +
        '<span class="exp-label">🔗 Cümle Pratiği &nbsp;' + (_st.idx+1) + ' / ' + _st.words.length + '</span>' +
        '<button class="exp-x" id="exp-x">✕</button>' +
      '</div>' +
      '<div class="exp-bar"><div class="exp-bar-fill" style="width:' + pct + '%"></div></div>' +
      '<div class="exp-card">' +
        '<div class="exp-icon">' + (w.icon||'📖') + '</div>' +
        '<div class="exp-en">' + w.en + '</div>' +
        '<div class="exp-tr">' + (w.tr||'') + '</div>' +
        (w.ipa ? '<div class="exp-ipa">' + w.ipa + '</div>' : '') +
      '</div>' +
      '<div class="exp-hint">Cümleyi sıraya koy</div>' +
      '<div class="exp-drop" id="exp-drop"></div>' +
      '<div class="exp-pool" id="exp-pool"></div>' +
      '<div class="exp-fb" id="exp-fb"></div>' +
      '<div class="exp-row">' +
        '<button class="exp-btn-skip" id="exp-skip">Geç</button>' +
        '<button class="exp-btn-go" id="exp-go">Kontrol Et</button>' +
      '</div>';
    document.body.appendChild(div);

    // animate in
    requestAnimationFrame(function(){ div.style.opacity='1'; div.style.transform='translateY(0)'; });

    document.getElementById('exp-x').addEventListener('click', close);
    document.getElementById('exp-go').addEventListener('click', _check);
    document.getElementById('exp-skip').addEventListener('click', function(){ _st.idx++; _render(); });
    _renderChips();
  }

  function _renderChips(){
    var pool = document.getElementById('exp-pool');
    var drop = document.getElementById('exp-drop');
    if(!pool||!drop) return;

    pool.innerHTML = _st.remaining.map(function(item,i){
      return '<button class="exp-chip" data-pi="'+i+'">'+item.w+'</button>';
    }).join('');
    drop.innerHTML = _st.placed.length
      ? _st.placed.map(function(item,i){
          return '<button class="exp-chip exp-placed" data-di="'+i+'">'+item.w+'</button>';
        }).join('')
      : '<span class="exp-ph">Kelimelere dokun →</span>';

    drop.querySelectorAll('.exp-chip').forEach(function(btn){
      btn.addEventListener('click', function(){
        if(_st.checked) return;
        var i = parseInt(btn.dataset.di);
        _st.remaining.push(_st.placed.splice(i,1)[0]);
        _renderChips();
      });
    });
    pool.querySelectorAll('.exp-chip').forEach(function(btn){
      btn.addEventListener('click', function(){
        if(_st.checked) return;
        var i = parseInt(btn.dataset.pi);
        _st.placed.push(_st.remaining.splice(i,1)[0]);
        _renderChips();
      });
    });
  }
  ...
  function _check(){
    var btn = document.getElementById('exp-go');
    if(_st.checked){ _st.idx++; _render(); return; }

    var w = _st.words[_st.idx];
    var correct = (w.ex||'').replace(/[.!?]+$/,'').split(/\s+/);
    var user    = _st.placed.map(function(x){ return x.w; });
    var ok = user.length === correct.length &&
             user.every(function(x,i){ return x.toLowerCase() === correct[i].toLowerCase(); });

    _st.checked = true;
    if(ok) _st.score++;

    var fb   = document.getElementById('exp-fb');
    var drop = document.getElementById('exp-drop');
    if(fb)  { fb.textContent = ok ? '✅ Doğru!' : '❌ Doğru sıra: ' + w.ex; fb.className = 'exp-fb ' + (ok?'exp-ok':'exp-err'); }
    if(drop) drop.className = 'exp-drop ' + (ok?'exp-drop-ok':'exp-drop-err');
    if(btn)  btn.textContent = 'Devam →';
    if(navigator.vibrate) navigator.vibrate(ok ? 40 : [80,50,80]);
  }

  function _showResult(){
    var old = document.getElementById('exp-overlay');
    if(old) old.parentNode && old.parentNode.removeChild(old);
    var total = _st.words.length;
    var pct = total ? Math.round(_st.score/total*100) : 0;
    var icon = pct>=80 ? '🌟' : pct>=50 ? '💪' : '📚';
    var div = document.createElement('div');
    div.id = 'exp-overlay';
    div.className = 'exp-overlay exp-result-screen';
    div.innerHTML =
      '<div class="exp-res-icon">'+icon+'</div>' +
      '<div class="exp-res-title">Pratik Bitti!</div>' +
      '<div class="exp-res-score">'+_st.score+' / '+total+' doğru &nbsp;('+pct+'%)</div>' +
      '<div class="exp-row" style="justify-content:center;gap:12px">' +
        '<button class="exp-btn-go" id="exp-again">Tekrar</button>' +
        '<button class="exp-btn-skip" id="exp-close2">Kapat</button>' +
      '</div>';
    document.body.appendChild(div);
    requestAnimationFrame(function(){ div.style.opacity='1'; div.style.transform='translateY(0)'; });
    document.getElementById('exp-again').addEventListener('click', function(){ _st.idx=0; _st.score=0; _render(); });
    document.getElementById('exp-close2').addEventListener('click', close);
  }

  window._exPractice = { start: start };

  // Sinestezi bitiş ekranına buton ekle
  var _obs = new MutationObserver(function(){
    var actions = document.querySelector('.synth-finish-actions');
    if(actions && !actions.querySelector('.exp-launch-btn')){
      var btn = document.createElement('button');
      btn.className = 'synth-hint-btn exp-launch-btn';
      btn.style.cssText = 'margin-top:8px;background:rgba(124,58,237,0.12);border-color:rgba(124,58,237,0.35);color:#a78bfa';
      btn.textContent = '🔗 Cümle Pratiği';
      btn.addEventListener('click', function(){
        var pool = window._app && window._app.session && window._app.session.learnPool;
        if(pool && pool.length) start(pool);
      });
