...
// A1 extra questions (30 new, bringing A1 from 20→50)
const a1Extra = [
...
// Load the remaining levels from JSON
const rest = JSON.parse(fs.readFileSync(path.join(__dirname, 'gq_extra.json'), 'utf8'));
const allExtra = [...a1Extra, ...rest];

// Verify counts
const counts = {};
allExtra.forEach(q => counts[q.level] = (counts[q.level]||0)+1);
console.log('Extra question counts:', counts);
...
console.log('Inserting', allExtra.length, 'questions at index', end);

const insertion = ',' + allExtra.map(serialize).join(',');
...
console.log('Total Grammar Quest questions:', total);
