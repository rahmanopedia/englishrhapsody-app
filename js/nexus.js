class NexusMode {
  ...
  init(rootEl) {
    ...
    // Delegated event handlers for nexus buttons (data-attribute pattern)
    if (!window._nexusDelegateAttached) {
      window._nexusDelegateAttached = true;
      document.addEventListener('click', (e) => {
        if (!window.nexusMod) return;

        const startNetwork = e.target.closest('.nexus-start-network-btn');
        if (startNetwork) { window.nexusMod.startNetwork(); return; }

        const startCipher = e.target.closest('.nexus-start-cipher-btn');
        if (startCipher) { window.nexusMod.startCipher(); return; }

        const startSynthesis = e.target.closest('.nexus-start-synthesis-btn');
        if (startSynthesis) { window.nexusMod.startSynthesis(); return; }

        const clearSlotBtn = e.target.closest('.nexus-clear-slot-btn');
        if (clearSlotBtn) { window.nexusMod.clearSynthSlot(clearSlotBtn.dataset.slotType); return; }

        const selectOrbBtn = e.target.closest('.nexus-select-orb-btn');
        if (selectOrbBtn) { window.nexusMod.selectSynthOrb(selectOrbBtn.dataset.orbType, selectOrbBtn.dataset.orbWord, selectOrbBtn.dataset.orbId); return; }

        const coreSpeakBtn = e.target.closest('.nexus-core-speak-btn');
        if (coreSpeakBtn) { window.nexusMod.playVerbAudio(coreSpeakBtn.dataset.verb); return; }

        const particleAnswerBtn = e.target.closest('.nexus-particle-answer-btn');
        if (particleAnswerBtn) { window.nexusMod.checkNetworkAnswer(particleAnswerBtn.dataset.particle, particleAnswerBtn); return; }

        const scrollDialBtn = e.target.closest('.nexus-scroll-dial-btn');
        if (scrollDialBtn) { window.nexusMod.scrollDial(scrollDialBtn.dataset.dialType, parseInt(scrollDialBtn.dataset.dialDir)); return; }

        const checkCipherBtn = e.target.closest('.nexus-check-cipher-btn');
        if (checkCipherBtn) { window.nexusMod.checkCipher(); return; }

        const restartBtn = e.target.closest('.nexus-restart-btn');
        if (restartBtn) { window.nexusMod.init(window.nexusMod.root); return; }
      });
    }
  }
    ...
    this.root.innerHTML = `
      ...
      <div class="synth-game-container animate-in">
         ...
         <div class="synth-pool">
            ${this.currentSynthPool.verbs.map((v, i) => `<div class="synth-orb verb nexus-select-orb-btn" id="sorb-v-${i}" data-orb-type="verb" data-orb-word="${v.replace(/"/g,'&quot;')}" data-orb-id="sorb-v-${i}">${v}</div>`).join('')}
            ${this.currentSynthPool.parts.map((p, i) => `<div class="synth-orb particle nexus-select-orb-btn" id="sorb-p-${i}" data-orb-type="particle" data-orb-word="${p.replace(/"/g,'&quot;')}" data-orb-id="sorb-p-${i}">${p}</div>`).join('')}
         </div>
      </div>
  ...
  checkSynthesis() {
      ...
      if (isVerbCorrect && isParticleCorrect) {
          ...
          if (sentBox) {
            sentBox.textContent = '';
            const _exText = document.createTextNode('"' + this.current.ex + '"');
            sentBox.appendChild(_exText);
            const _badge = document.createElement('div');
            _badge.style.cssText = 'color:#ec4899; margin-top:20px; font-size:1.5rem; font-weight:800; letter-spacing:3px;';
            _badge.textContent = 'FÜZYON BAŞARILI!';
            sentBox.appendChild(_badge);
          ...
          this.score += (base * 3) + (this.combo * 10);
          this.combo++;          this._updateHUD();
    ...
    this.root.innerHTML = `
      ...
      <div class="nexus-game-area" id="nexus-board">
        ...
        ${particlesToShow.map((opt, i) => {
           const fs = opt.length <= 4 ? '0.95rem' : opt.length <= 7 ? '0.78rem' : '0.65rem';
           return `<div class="nexus-particle-node nexus-particle-answer-btn" id="particle-${i}" style="font-size:${fs}" data-particle="${opt.replace(/"/g,'&quot;')}">${opt}</div>`;
        }).join('')}
      </div>
  ...
  checkNetworkAnswer(particle, el) {
    ...
    if (particle === task.particle) {
       ...
       const base = window.remoteFlags?.xp_nexus_base ?? 15;
       this.score += (base * 2) + (this.combo * 5);
       this.combo++;
       this._updateHUD();       
  ...
  _renderCipher() {
    ...
    this.root.innerHTML = `
      ...
      <button class="cipher-engage-btn nexus-check-cipher-btn" id="cipher-engage">ŞİFREYİ ÇÖZ</button>
  ...
  checkCipher() {
      ...
      if (isVerbCorrect && isParticleCorrect) {
          ...
          sentBox.textContent = '';
          const _cipherText = document.createTextNode('"' + this.current.ex + '"');
          sentBox.appendChild(_cipherText);
          const _cipherBadge = document.createElement('div');
          _cipherBadge.style.cssText = 'color:#10b981; margin-top:15px; font-size:1.3rem; font-weight:800; text-transform:uppercase; letter-spacing:2px;';
          _cipherBadge.textContent = '🔐 ŞİFRE KIRILDI!';
          sentBox.appendChild(_cipherBadge);
  ...
  _showResults(title) {
    const mult = window.remoteFlags?.multiplier_medium ?? 1.5;
    ...
    this.root.innerHTML = `
      ...
      <div class="nexus-game-area" style="text-align:center;">
        ...
        <p style="color:var(--text-2); font-size:1.2rem; margin-bottom:30px;">Kazanılan Toplam XP: <strong style="color:var(--cyan)">+${finalXP}</strong></p>
        <div style="display:flex; gap:15px; justify-content:center;">
           <button class="btn btn-primary nexus-intro-btn nexus-restart-btn">MOD SEÇİMİ</button>
